...
import uno.dos.tres.service.Validator;
...
public class ValidatorImpl implements Validator {
    ...
    @Override
    public void checkRegInfo(RegInfo regInfo) throws ServiceException {
        String userName = regInfo.getUserName();
        String password = regInfo.getPassword();
        String email = regInfo.getEmail();
            if (!usernameValidator.isValid(userName)){
                throw new ServiceException("Invalid username. Come up with a username from 2 to 18 characters");
            } else if (!emailValidator.isValid(email)){
                throw new ServiceException("Invalid email");
            } else if (!passwordValidator.isValid(password)){
                throw new ServiceException("Invalid password. Please use a password of at least 8 characters, as well as numbers, uppercase and lowercase letters");
            }
    }
}
